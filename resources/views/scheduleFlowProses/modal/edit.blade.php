    ...
    <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Ubah Plan</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="{{ route('schedule.update', $flowproses->id) }}" method="POST">
                    @csrf
                    @method('put')
                            ...
                            <tbody>
                                @foreach ($flowproses->flowprosesdetails as $detail)
                                    <tr>
                                        ...
                                        <input type="hidden" name="schedule_id[]" value="{{ $detail->schedule->id }}">
                                        <td class="text-center"><input type="date" class="form-control"
                                                name="planningTanggal[]"
                                                value="{{ $detail->schedule->planningTanggal }}"
